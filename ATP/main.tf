resource "random_string" "autonomous_database_atp_admin_password" {
  length      = 16
  min_numeric = 1
  min_lower   = 1
  min_upper   = 1
...
resource "random_integer" "atp_priority" {
  min = 1
  max = 100
}

resource "random_string" "atp_random" {
  length = 2
  special = false
  number = true
}

resource "oci_database_autonomous_database" "atp_autonomous_database" {
  #Required
  admin_password = random_string.autonomous_database_atp_admin_password.result
  ...
  db_name                  = "dish5Gatp${random_string.atp_random.result}db"
  db_version               = "19c"
  db_workload              = "OLTP"
  display_name             = "dish5Gatp${random_integer.atp_priority.result}db"
